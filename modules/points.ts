...
const table = new Table({
    head: ['#', 'Address', 'Balance', 'Balance USD', 'Fractal Balance', 'Points', "F Points"],
    colWidths: [5, 65, 20, 20, 20, 10, 10],
});
...
async function getPoints(wallet: Wallet, network: string) {
...
async function pointsChecker(): Promise<void> {
        ...
        const batchPromises = batch.map(async ([seed, proxy], index) => {
            ...
            if (mainnetPoints?.msg === "ok") {
                ...
                const row = [
                    ...
                    mainnetPoints.data.inscribeCount,
                    fractalPoints.data.inscribeCount,
                ];
    ...
    fastcsv.write(csvData, { headers: ['#', 'Address', 'Balance BTC', 'Balance USD', 'Fractal Balance', 'Points', "F Points"] }).pipe(ws);
